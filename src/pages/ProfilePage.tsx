...
export const ProfilePage: React.FC = () => {
  ...
  const isMyProfile = !urlUserId || Number(urlUserId) === Number(currentUser?.userId || currentUser?.id);

  useEffect(() => {
    const loadProfile = async () => {
      if (urlUserId && Number(urlUserId) !== Number(currentUser?.userId || currentUser?.id)) {
        try {
          setFetchingUser(true);
          const userData = await UserAPI.getInfoById(urlUserId);
    ...
    <div className="h-full flex flex-col gap-6 overflow-y-auto pb-10 pr-4 scrollbar-thin scrollbar-thumb-slate-200">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">
            {isMyProfile ? '个人中心' : '用户资料'}
          </h1>
          <p className="text-slate-500 font-medium mt-1">
            {isMyProfile ? '管理您的个人信息与账号安全' : `正在查看 ${profileUser?.name || profileUser?.username || '用户'} 的资料`}
          </p>
        </div>
        {isMyProfile ? (
          !isEditing ? (
          ...
          <button
            ...
            className="bg-indigo-50 hover:bg-indigo-100 text-indigo-600 px-6 py-2.5 rounded-2xl font-black transition-all flex items-center gap-2 border border-indigo-100 shadow-sm shadow-indigo-100/30"
          >
            <ArrowLeft size={18} /> 退出查看
          </button>
        )}
      </div>
      ...
      {(fetchingUser) ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-slate-400 font-bold">正在加载用户资料...</p>
        </div>
      ) : !profileUser ? (
        <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
          <p className="text-slate-400 font-bold text-lg">未找到该用户资料</p>
          <button onClick={() => navigate('/profile')} className="mt-4 text-blue-600 font-black hover:underline">返回我的个人中心</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
          ...
          <div className="xl:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col items-center relative overflow-hidden text-center">
              ...
              <div className="relative mt-8 mb-4 group">
                <div className="w-32 h-32 rounded-full border-4 border-white bg-white shadow-lg overflow-hidden transition-transform duration-300 group-hover:scale-105">
                  <img
                    src={(isEditing ? editForm.avatarUrl : (profileUser?.avatar || profileUser?.avatarUrl)) || undefined}
                    alt="Avatar"
                    className={`w-full h-full object-cover transition-opacity duration-300 ${(!(isEditing ? editForm.avatarUrl : (profileUser?.avatar || profileUser?.avatarUrl))) ? 'opacity-0' : 'opacity-100'}`}
                  />
                  {(!(isEditing ? editForm.avatarUrl : (profileUser?.avatar || profileUser?.avatarUrl))) && (
              ...
              <h2 className="text-2xl font-black text-slate-800 flex items-center justify-center gap-2">
                {profileUser?.realname || profileUser?.username}
                <BadgeCheck size={20} className="text-blue-500" />
              </h2>
              <p className="text-slate-500 font-medium text-sm mt-1">{profileUser?.role || '普通成员'}</p>
              ...
              <div className="w-full space-y-4 text-left">
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-slate-700">{profileUser?.sduId || '未绑定'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-slate-700">{profileUser?.createdAt ? new Date(profileUser.createdAt).toLocaleDateString() : '未知'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-emerald-600 flex items-center gap-1.5">
                      ...
                      {profileUser?.isActive !== false ? '正常激活' : '已停用'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {/* Security Action (Only for My Profile) */}
            {isMyProfile && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        type="text"
                        value={isEditing ? editForm.username : (profileUser?.username || '未设置')}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="text"
                        value={profileUser?.realname || '未认证'}
                        disabled
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        type="email"
                        value={isEditing ? editForm.email : (profileUser?.email || '')}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        type="text"
                        value={isEditing ? editForm.department : (profileUser?.department || '')}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        type="text"
                        value={isEditing ? editForm.role : (profileUser?.role || '普通成员')}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-2">
                  {searchResults.map(u => (
                    <div
                      ...
                      className="flex items-center gap-4 p-3 border border-slate-100 bg-slate-50/50 rounded-2xl hover:bg-white hover:shadow-md hover:border-blue-100 transition-all cursor-pointer group"
                    >
                      ...
                      <span className="text-[10px] font-black bg-blue-100 text-blue-700 px-2 py-1 rounded-lg group-hover:bg-blue-600 group-hover:text-white transition-colors">
                        查看
                      </span>
                    </div>
                  ))}
