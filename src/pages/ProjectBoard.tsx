...
export const ProjectBoard: React.FC = () => {
  ...
  const project = state.projects.find(p => (p.id === pId) || (p.projectId === pId));
    ...
    return (
      <div key={task.id} className="bg-white p-3 rounded-lg shadow-sm border border-slate-200 mb-3 hover:shadow-md transition-shadow">
        <div className="flex justify-between items-start mb-2">
          <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${task.priority === TaskPriority.HIGH ? 'bg-red-100 text-red-600' :
      ...
      {isTaskModalOpen && (
        <CreateTaskModal
          ...
          onSubmit={async (data) => {
            ...
            // Refresh tasks locally or re-fetch from backend
            // Assuming backend returns info, if not we create a local mock equivalent for instant update
            const newTask: Task = {
              id: taskId,
              taskId: taskId,
              title: data.title,
